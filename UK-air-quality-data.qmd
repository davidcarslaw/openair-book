...
In the example below the unique sites are selected from `aurn_detailed`
because the `site` repeats the number of pollutants that are measured.
Information is also collected for the map popups and then the map is
plotted.

```{r}
#| label: fig-leafletmap
#| cache: false
#| fig-cap: Plotting sites on an interactive map.
#| results: asis
library(leaflet)
