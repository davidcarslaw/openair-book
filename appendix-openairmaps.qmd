...
-   `provider`: Any number of the [leaflet]{.pkg} providers (see `leaflet::providers`).

Some of these arguments are demonstrated in @fig-networkMap2, which shows the AURN, AQE Network, and locally managed English networks[^appendix-openairmaps-1]. Pay particular attention to the layer control menu, which allows you to toggle different site types on and off.

[^appendix-openairmaps-1] Note that networks can overlap --- AQE sites can be part of the AURN. `networkMap()` uses the user-provided order as a hierarchy, so if "aurn" is listed first, an AURN & AQE site will be labelled as part of the AURN and its AURN code will be displayed.
...
-   `control`: A column to use to create a "layer control" menu. Specifying `control` effectively splits the input data along the specified column, creating multiple separate sets of directional analysis plots. Common columns to pass to `control` will be those created by `openair::cutData()` or `openair::splitByDate()`.[^appendix-openairmaps-2]

[^appendix-openairmaps-2]: Note that maps can only have one "layer control" menu. Users should therefore only provide multiple pollutants *or* an argument to `control`, but never both. If multiple pollutants *and* `control` are specified, `control` will be ignored.
