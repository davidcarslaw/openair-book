...
```{r summaryPolar}
#| label: tbl-polardata
#| tbl-cap: "A statistical summary of the"
polar_data %>%
  select(-site_type) %>%
...
-   `latitude`, `longitude`: The lat/lon column names.

-   `type`: The `type` column should distinguish between different lat/lon combinations. Commonly this will be "site", "site_code", "site_name" or something to that effect!
...
polarMap(polar_data, pollutant = "nox", type = "site")
```
...
-   Letting users switch between different years, to see how pollutant sources might have changed over time.
...
By default, `trajMap()` will plot your trajectories in black on a default "OpenStreetMap" base map, as shown in @fig-trajMap. A popup is also automatically generated --- try clicking on the points to get information about that particular air mass.
