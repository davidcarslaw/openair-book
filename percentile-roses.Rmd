...
opts_chunk$set(fig.shofig.width='asis',warning=FALSE, crop=TRUE, background=c(1,1,1), cache=TRUE, tidy=FALSE)
```
...
probability of doing so. In **openair**, a CPF plot can be produced as
shown in \@ref(fig:percentileRoseCPF). Note that in these plots
only one percentile is provided and the method must be supplied. In
