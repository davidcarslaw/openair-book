...
```{r traj1,w=6,h=6,out.width='100%',fig.cap='96-hour Hysplit back trajectories centred on London for 7 days in April 2010. Note the additional option to vary the country colours using `map.cols`. By default the map colours for all countries are grey.',message=FALSE,warning=FALSE}
trajPlot(selectByDate(traj, 
                      start = "15/4/2010", 
                      end ="21/4/2010"),
...
```{r traj3,w=6,h=7,out.width='100%',fig.cap='96-hour Hysplit back trajectories centred on London for 7 days in April 2010, shown grouped for each day and coloured accordingly.'}
trajPlot(selectByDate(traj, 
                      start = "15/4/2010", 
                      end = "21/4/2010"),
         group = "day", col = "jet", 
...
```{r traj4,w=6,h=6,out.width='100%',fig.cap='(ref:Traj4)'}
trajPlot(selectByDate(traj, 
                      start = "15/4/2010", 
                      end = "21/4/2010"),
         pollutant = "pm10", 
...
```{r trajFreq,w=8,h=8,out.width='100%',fig.cap='Gridded back trajectory frequencies. The `border = NA` option removes the border around each grid cell.'}
...
```{r trajHex,w=8,h=5,out.width='100%',fig.cap='Gridded back trajectory frequencies with hexagonal binning.'}
trajLevel(filter(traj, lat > 30, lat < 70, lon > -30, lon < 20),
...
```{r trajDiff,fig.width=10,fig.height=6,out.width='100%',fig.cap='Gridded back trajectory frequencies showing the percentage difference in occurrence for high PM~10~ concentrations (90th percentile) compared with conditions over the full year.'}
...
```{r PSCF,fig.width=10,fig.height=6,out.width='100%',fig.cap='PSCF probabilities for PM~2.5~ concentrations (90th percentile).'}
...
```{r traj5,fig.width=10,fig.height=6,out.width='100%',fig.cap='Gridded back trajectory concentrations showing mean PM~2.5~ concentrations using the CWT approach.'}
trajLevel(filter(traj, lon > -20, lon < 20, lat > 45, lat < 60),
          pollutant = "pm2.5", 
          statistic="cwt", 
          col = "increment",
...
```{r traj6,fig.width=10,fig.height=6,out.width='100%',fig.cap='Gridded and smoothed back trajectory concentrations showing mean PM~2.5~ concentrations using the CWT approach.'}
...
```{r calcClusterTraj, fig.width=6, fig.height=5, out.width='100%',fig.cap='The 6-cluster solution to back trajectories calculated for the London North Kensington site for 2011 showing the mean trajectory for each cluster. '}
