...
-   `cluster`: By default, markers are clustered together until users zoom in close. This default behaviour improves the appearance and performance of the HTML map widget. The `cluster` argument allows you to turn this feature off.^[This feature cannot be turned off when `network = "europe"` due to the number of markers creating performance issues.]

-   `provider`: Any number of the [leaflet]{.pkg} providers (see `leaflet::providers` or the list in the [openairmaps overview page](maps-overview.qmd)).
