...
The `selectRunning` function returns a new column `criterion` that flags whether the condition is met or not. The user can control the text provided, which by defauly is "yes" and "no".

```{r criterion}
table(ted$criterion)
```

Now we are going to produce two pollution roses shown in Figure \@ref(fig:RunningRes). Note, however that many other types of analysis could be carried out now the data have been partitioned.

```{r RunningRes,fig.width=8,fig.height=5,out.width='90%',fig.cap='Example of using the `selectRunning` function to select episode hours to produce pollution roses of O~3~ concentration.'}
pollutionRose(ted, pollutant = "o3", 
          type = "criterion")
```

The results are shown in Figure \@ref(fig:RunningRes). The pollution rose for for the "no" criterion (left plot of Figure \@ref(fig:RunningRes)) shows that the highest O~3~ concentrations tend to occur for wind directions from the south-west, where there is a high proportion of measurements. By contrast, the  when the criterion is met (right plot of Figure \@ref(fig:RunningRes)) is very different. In this case there is a clear set of conditions where these criteria are met i.e. lengths of at least 8-hours where the O~3~ concentration is at least 90\~ppb. It is clear the highest concentrations are dominated by south-easterly conditions i.e. corresponding to easterly flow from continental Europe where there has been time to the O~3~ chemistry to take place.
