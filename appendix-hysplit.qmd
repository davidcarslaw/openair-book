...
As discussed in @sec-trajPlot, [openair]{.pkg} can import
...
1.  Download and install the [NOAA Hysplit model](https://www.ready.noaa.gov/HYSPLIT.php), somewhere with
...
It is first necessary on ensure
...
latter is needed to load some R functions stored as a *GitHub
  gist* (some code that can be shared publicly). Copy the code below and paste it into R, which will load the necessary functions needed to run Hysplit.
