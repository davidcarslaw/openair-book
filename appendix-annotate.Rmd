...
\@ref(fig:timePlotAnnotate), which is an enhanced version of Figure
\@ref(fig:timePlot). These annotations have been subsequently added
...
`latticeExtra} package, which should already be installed as part
...
```{r timePlotAnnotate,fig.keep='last',echo=FALSE, fig.width=10, fig.height=8}
library(openair)
...
```{r annonatePolar, fig.keep='last', fig.width=5,fig.height=5,out.width='50%', echo = FALSE}
arc <- function(theta1 = 30, theta2 = 60, theta3 =theta1, theta4 = theta2,
                lower = 1, upper = 10){
  ## function to work out coordinates for an arc sector
  if (theta2 < theta1) {
...
\@ref(fig:annotatePolar). Note that in these plots (0, 0) is the
...
Given a plot such as Figure \@ref(fig:timeVar1), how could texts be added
...
First, make a `timeVariation` plot like Figure \@ref(fig:timeVar1).
