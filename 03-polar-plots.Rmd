# The `polarPlot` and `polarCluster` functions
...
ways as described below. Recent publications that describe or use the
...
intervals at 10$^\circ$ and 40 wind speed intervals capture sufficient
...
30 m s^-1^, intervals greater than 30 would be difficult to justify based
...
with either Figure \@ref(fig:polarPlot2) or Figure \@ref(fig:polarPlot) is
that the concentration pattern is very different i.e. high
...
A polar plot of the SO~2~/NO~x~ ratio is shown in
...
ratio. This is easy to do with the `filter` command. The code below
selects ratios less than 0.1.
...
a particular layout see \autopageref{sec:multiple-plots-page}.
...
conditions together. \@ref(sec:trajCluster) provides the details of
clustering back trajectories in **openair**. A fuller description of the
...
The example given below is for concentrations of SO~2~, shown in
...
```{r clustTV, eval=FALSE}
timeVariation(subset(results, cluster %in% c("C5", "C6")),
              pollutant="so2",
              group = "cluster",  
