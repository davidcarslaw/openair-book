...
# Wind and Pollution Roses {#windRose}
...
\@ref(fig:windroseBasic).

```{r windroseBasic,fig.width=5,fig.height=5,out.width='60%',fig.cap='Use of `windRose` function to plot wind speed/direction frequencies. Wind speeds are split into the intervals shown by the scale in each panel. The grey circles show the % frequencies.'}
windRose(mydata)
```
...
when the wind was from the SSW and 2003 clearly had more occasions when
the wind was easterly. It can also be useful to use `type = "month"` to
get an idea of how wind speed and direction vary seasonally.

```{r windRoseyear,fig.width=8,fig.height=5,out.width='100%',fig.cap='Use of `windRose` function to plot wind speed/direction frequencies by year. Wind speeds are split into the intervals shown by the scale in each panel. The grey circles show the 10 and 20% frequencies.'}
windRose(mydata, type = "year", layout = c(4, 2))
```

The `type` option is very flexible in [openair]{.pkg} and can be used to
quickly consider the dependencies between variables. Section
\@ref(cutData) describes the basis of this option in [openair]{.pkg}
plot. As an example, consider the question: what are the meteorological
conditions that control high and low concentrations of PM~10~? By
setting `type = "pm10"`, [openair]{.pkg} will split the PM~10~
concentrations into four *quantiles* i.e. roughly equal numbers of
points in each level. The plot will then show four different wind roses
for each quantile level, although the default number of levels can be
set by the user --- see `?cutData` for more details. Figure
\@ref(fig:windRsPM10) shows the results of setting `type = "pm10"`. For
the lowest concentrations of PM~10~ the wind direction is dominated by
northerly winds, and relatively low wind speeds. By contrast, the
highest concentrations (plot furthest right) are dominated by relatively
strong winds from the south-west. It is therefore very easy to obtain a
good idea about the conditions that tend to lead to high (or low)
concentrations of a pollutant. Furthermore, the `type` option is
available in almost all [openair]{.pkg} functions.

```{r windRsPM10,fig.width=10,fig.height=4,out.width='100%',fig.cap='Wind rose for four different levels of PM~10~ concentration. The levels are defined as the four *quantiles* of PM~10~ concentration and the ranges are shown on each of the plot labels.'}
...
function in recent versions of [openair]{.pkg}, e.g. as in Figure
\@ref(fig:pollutionRoseBasic):

```{r pollutionRoseBasic,fig.width=5,fig.height=4,out.width='60%',fig.cap='NO~x~ pollution rose produced using  `pollutionRose` and default `pollutionRose` settings.'}
...
While Figure \@ref(fig:pollutionRoseBasic) indicates that higher NO~x~
concentrations are also associated with the SW, conditioning allows you
to be much informative. For example, conditioning by SO~2~ (Figure
\@ref(fig:pollRoseNOxO2) demonstrates that higher NO~x~ concentrations
are associated with the SW and much of the higher SO~2~ concentrations.
However, it also highlights a notable NO~x~ contribution from the E,
most apparent at highest SO~2~ concentrations that is obscured in Figure
\@ref(fig:pollutionRoseBasic) by a relatively high NO~x~ background
(Figure \@ref(fig:pollRoseNOxO2).

```{r pollRoseNOxO2,fig.width=10,fig.height=5,out.width='100%',fig.cap='NO~x~ pollution rose conditioned by SO~2~ concentration.'}
pollutionRose(mydata, pollutant = "nox", 
              type = "so2", 
              layout = c(4, 1),
              key.position = "bottom")
...
different concentration levels. A simple plot is shown in Figure
\@ref(fig:pollRoseAttr), which clearly shows the dominance of
south-westerly winds controlling the overall mean NO~x~ concentrations
at this site. Indeed, almost half the overall NO~x~ concentration is
contributed by two wind sectors to the south-west. The `polarFreq`
function can also show this sort of information, but the pollution rose
is more effective because both length and colour are used to show the
contribution. These plots are very useful for understanding which wind
directions control the overall mean concentrations.

```{r pollRoseAttr,fig.width=5,h=5,out.width='60%',fig.cap='Pollution rose showing which wind directions contribute most to overall mean concentrations.'}
...
clearly. An example is shown in Figure \@ref(fig:pollRoseNorm) where for
easterly winds it is now clearer that a greater proportion of the time
the concentration is made up of high SO~2~ concentrations. In this plot
each wind sector is scaled between 0 and 1. Also shown with a black like
is an indication of the wind direction frequency to remind us that winds
from the east occur with a low frequency.

```{r pollRoseNorm,fig.width=5,h=4,out.width='60%',fig.cap='SO~2~ pollution rose produced using `pollutionRose` normalised by each wind sector.'}
...
## Comparing two meteorological data sets {#comp-met}
...
\@ref(fig:pollRoseComp) is the mean wind speed and direction bias as
numerical values.

Note that the `type` option can be used in Figure
\@ref(fig:pollRoseComp) e.g. `type = "month"` to split the analysis in
useful ways. This is useful if one wanted to see whether a site or the
output from a model was biased for different periods. For example,
`type = "daylight"` would show whether there are biases between
...
including one that spans -0.5 to +0.5 that is useful to show wind speeds
that do not change.
