...
@droppo2008 and @applequist2012.^[Thanks to Philippe
  Barn\'eoud of Environment Canada for pointing this issue out.]
...
\autoref{fig:windRoseyear} highlights some interesting differences
...
\begin{figure}
 ```{r windRoseyear,fig.width=10,h=5,out.width='1\\textwidth'}
windRose(mydata, type = "year", layout = c(4, 2))
```
\caption{Use of `windRose} function to plot wind
...
variables. \autoref{sec:cutData} describes the basis of this option in
...
\pmten? By setting `type = "pm10"}, **openair** will split the \pmten
...
details. \autoref{fig:windRsPM10} shows the results of setting
`type = "pm10"}. For the lowest concentrations of \pmten the wind
...
\begin{figure}
```{r windRsPM10,fig.width=10,h=4,out.width='1\\textwidth'}
windRose(mydata, type = "pm10", layout = c(4, 1))
```
\caption{Wind rose for four different levels of \pmten
  concentration. The levels are defined as the four *quantiles*
  of \pmten concentration and the ranges are shown on each of the plot
labels.}
{#fig:windRsPM10}
\end{figure}
...
that are due to the bias issue discussed by \textcite{applequist2012}.
...
pollutant species, as demonstrated by Ronald Henry and co-authors in \textcite{henry2009}.

You can produce similar pollution roses using the `pollutionRose}
function in recent versions of **openair**, e.g. as in
\autoref{fig:pollutionrose1}:

\begin{figure}
```{r pollutionRose1,fig.width=5,h=4,out.width='.45\\textwidth'}
pollutionRose(mydata, pollutant = "nox")
```
\caption{\nox pollution rose produced using
    `pollutionRose} and default `pollutionRose} settings.}
    {#fig:pollutionrose1}
\end{figure}
...
While \autoref{fig:pollutionrose1} indicates that higher \nox
concentrations are also associated with the SW, conditioning allows
you to be much informative. For example, conditioning by \sotwo
(\autoref{fig:pollutionrose2}) demonstrates that higher \nox
concentrations are associated with the SW and much of the higher
\sotwo concentrations. However, it also highlights a notable \nox
contribution from the E, most apparent at highest \sotwo
concentrations that is obscured in \autoref{fig:pollutionrose1} by a
relatively high \nox background (\autoref{fig:pollutionrose2}).

\begin{figure}
```{r pollutionRose2,fig.width=10,h=5,out.width='1\\textwidth'}
pollutionRose(mydata, pollutant = "nox", type = "so2", layout = c(4, 1))
```
\caption{\nox pollution rose conditioned by \sotwo concentration.}
    {#fig:pollutionrose2}
\end{figure}
...
shown in \autoref{fig:pollRoseAttr}, which clearly shows the dominance
...
\begin{figure}
```{r pollRoseAttr,fig.width=5,h=5,out.width='.45\\textwidth'}
pollutionRose(mydata, pollutant = "nox", statistic = "prop.mean")
```
\caption{Pollution rose showing which wind directions contribute most
  to overall mean concentrations.}
{#fig:pollRoseAttr}
\end{figure}
...
pollution rose of \sotwo there are few occurrences of easterly winds
...
more clearly. An example is shown in \autoref{fig:pollutionroseNorm}
where for easterly winds it is now clearer that a greater proportion
of the time the concentration is made up of high \sotwo
...
\begin{figure}
```{r pollutionRoseNorm,fig.width=5,h=4,out.width='.45\\textwidth'}
pollutionRose(mydata, pollutant = "so2", normalise = TRUE, seg = 1)
```
\caption{\sotwo pollution rose produced using
    `pollutionRose} normalised by each wind sector.}
...
\subsubsection*{Comparing two meteorological data sets}
{#sec:comp-met}
...
biases. It does also show the wind speed bias \emph{but only if there
...
results are shown in \autoref{fig:pollRoseComp}. The Figure clearly
...
\autoref{fig:pollRoseComp} is the mean wind speed and direction bias
as numerical values.

Note that the `type} option can be used in
\autoref{fig:pollRoseComp} e.g. `type = "month"} to split the
...
\begin{figure}
```{r pollutionRoseComp, fig.width=5,h=5,out.width='.45\\textwidth'}
...
\caption{Pollution rose showing the difference between two
  meteorological data sets. The colours are used to show whether data
  tend to be postively or negatively biased with respect to the
  reference data set.}
{#fig:pollRoseComp}
\end{figure}

An example of using user-supplied breaks is shown in
\autoref{fig:pollRoseComp2}. In this case six intervals are chosen
including one that spans $-$0.5 to $+$0.5 that is useful to show wind
speeds that do not change.

\begin{figure}
```{r pollutionRoseComp2, fig.width=8,h=5,out.width='.8\\textwidth'}
...
\caption{Pollution rose showing the difference between two
  meteorological data sets. The colours are used to show whether data
  tend to be postively or negatively biased with respect to the
  reference data set. In this case the example shows how to use
  user-defined breaks and split the data by day/night for a latitude
  assumed to be London. }
{#fig:pollRoseComp2}
\end{figure}
