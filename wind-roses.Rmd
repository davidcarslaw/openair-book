...
[openair]{.pkg} uses a simple method to correct for the bias by globally
...
[openair]{.pkg} plot. As an example, consider the question: what are the
meteorological conditions that control high and low concentrations of
PM~10~? By setting `type = "pm10"`, [openair]{.pkg} will split the PM~10~
...
(Figure \@ref(fig:pollRoseNOxO2) demonstrates that higher NO~x~
...
concentrations that is obscured in Figure \@ref(fig:pollutionRoseBasic) by a
relatively high NO~x~ background (Figure \@ref(fig:pollRoseNOxO2).
...
more clearly. An example is shown in Figure \@ref(fig:pollRoseNorm)
