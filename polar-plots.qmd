...
The function `polarMap` requires information on the site (name or code), latitude and longitude in addition to the usual information required by the `polarPlot` function. The package comes with some example data (`polar_data`) that can be used as a template for using different data.
...
Or with a different base map (there are many!). See the help file for `polarMap` on how to find out about and use different base maps. Note that if you are using the `importAURN` family of functions it is useful to add the option `meta = TRUE`, which will return the latitude and longitude of the site together with the air quality data.

```{r}
#| label: fig-interMap2
#| cache: false
#| fig-cap: Interactive map of polar plots with an alternative base map.
#| column: screen
polarMap(polar_data,
  latitude = "latitude",
  longitude = "longitude",
