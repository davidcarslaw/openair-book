...
The data consists of hourly observations of O~3~ in $\mu$g m^-3^ at 15 rural
...
help illustrate the purpose of the function. The results are shown in Figure \@ref(fig:conditionalQ1). An explanation of the Figure is given in its caption.
...
\@ref(sec:modStats). These approaches are very useful for comparing
...
fails. See Section \@ref(sec:cutData) for more details on the types of
...
derive from the Section \@ref(sec:modStats) function and include *MB*,
...
```{r eval=FALSE}
condDat <- readRDS(url("https://davidcarslaw.github.io/data/openair/condDat.rds"))
```
