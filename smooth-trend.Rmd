# Smooth trends  {#smoothTrend}

## Background {#SmoothTrendInfo}
...
Section \@ref(trends-gam) considers smooth trends in more detail and
...
```{r smoothTrendComp,fig.width=6,fig.height=5,fig.show='hold', out.width='50%',fig.cap='Examples of the `smoothTrend` function applied to    Marylebone Road'}
...
the `TheilSen` function. Figure \@ref(fig:smoothwd) shows the
result of applying this function to O~3~ concentrations. The code
that produced Figure \@ref(fig:smoothwd) was:

```{r smoothwd,fig.width=10,fig.height=10,fig.cap='Trends in O~3~ using the `smoothTrend` function applied to Marylebone Road. The shading shows the estimated 95% confidence intervals.'}
...
\@ref(fig:smoothwdSeason) all three pollutants are chosen and two
...
```{r smoothwdSeason,fig.width=14,fig.height=12,fig.cap='The `smoothTrend` function applied to three pollutants,  split by wind sector and season.'}
