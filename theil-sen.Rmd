...
pollution is the non-parametric *Mann-Kendall* approach
[@Hirsch1982]. @Wilcox2010 provides an excellent case for
...
1950 [@theil50;@sen1968]. It is one of those methods that
required the invention of fast computers to be practical. The basic
idea is as follows. Given a set of $n$ $x$, $y$ pairs, the slopes
between all pairs of points are calculated. Note, the number of slopes
can increase by $\approx$ $n^2$ so that the number of slopes can
...
means. Bootstrap resampling also provides the estimate of $p$ for the
slope.
...
the issue. In [openair]{.pkg} we follow the suggestion of @Kunsch1989
of setting the block length to $n^{1/3}$ where n is the length of the
time series.
...
example, is there any evidence that concentrations of \nox have
...
considering subsets of data can be very insightful.

Another aspect is that almost all trends are shown as mean
...
if one is interested in *understanding* why trends are as they
are, it can be helpful to consider how concentrations vary in other
ways. The trend functions in [openair]{.pkg} do just this. Trends can be
...
first. The `timeVariation` function are both useful to determine
whether there is a seasonal cycle that should be removed.
...
Because the function runs simulations to estimate the uncertainty in
...
O~3~ was +0.38 units (i.e. ppb) per year as an average over the
...
concentrations of O~3~ increased over the period. The plot together
with the summary results is shown in Figure \@ref(fig:TheilSen1). Note
...
It is also possible to calculate trends in many other ways e.g. by
...
depends on the direction of the wind. The `TheilSen` function
...
run eight times in total. Considering concentrations of O~3~ again,
the output is shown in Figure \@ref(fig:TheilSen2). Note that this plot is
...
immediately that there is essentially no trend in O~3~ for southerly
winds i.e. where the road itself has the strongest influence. On the
other hand the strongest evidence of increasing O~3~ are for
northerly winds, where the influence of the road is much less. The
reason that there is no trend in O~3~ for southerly winds is that
there is always a great excess of NO, which reacts with O~3~ to form
NO~2~. At this particular location it will probably take many more
years before O~3~ concentrations start to increase when the wind
direction is southerly. Nevertheless, there will always be *some*
hours that do not have such high concentrations of NO.

```{r TheilSen2,warning=FALSE,fig.width=10,fig.height=10,fig.cap='Trends in ozone at Marylebone Road split by eight wind sectors. The `TheilSen` function will automatically organise the separate panels by the different compass directions.'}
...
The option `slope.percent` can be set to express slope
...
where $C_{End}$ and $C_{Start}$ are the mean concentrations for the
end and start date, respectfully. $N_{years}$ is the
number of years (or fractions of) the time series spans.


```{r MannKen, eval=FALSE}
TheilSen(mydata, pollutant = "o3", deseason = TRUE,
            slope.percent = TRUE)
```

The `TheilSen` function was written to work with hourly data,
...
data directly. *However, it is necessary to ensure the date field
  is in the correct format*. Assuming data in an Excel file in the
...
`as.Date`. Note that if dates are pre-formatted as YYYY-mm-dd,
then it is sufficient to use `as.Date` without providing any
format information because it is already in the correct format.

```{r eval=FALSE}
mydata$date = as.Date(mydata$date, format = "%d/%m/%Y")
```

Finally, the `TheilSen` function can consider trends at
...
and *site*. The call would then be, for example:
...
```{r MKout,fig.keep='none'}
MKresults <-  TheilSen(mydata, pollutant = "o3", deseason = TRUE, type = "wd")
```
This returns a list of two data frames containing all the monthly mean values and
trend statistics and an aggregated summary. The first 6 lines are shown next:
...
In the results above the `lower` and `upper` fields provide the
95% (or chosen confidence interval using the `alpha` option) of
the trend and `slope` is the trend estimate expressed in units/year.
