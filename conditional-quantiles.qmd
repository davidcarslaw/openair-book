...
help illustrate the purpose of the function. The results are shown in @fig-conditionaQ1. An explanation of the Figure is given in its caption.
...
the `type` option. The results are shown in @fig-condQall. It is now easier to see where the model
...
that site and setting `type = "season"`, as shown in @fig-conditionalSeas. These results show that winter
...
including the use of common statistical measures described in @sec-modStats. These approaches are very useful for comparing
...
fails. See @sec-cutData for more details on the types of
...
derive from the @sec-modStats) function and include *MB*,
...
depend on values of that variable. For example, in @fig-condDatWS it can be seen how wind speed varies across the
...
In an extension to @fig-condDatWS it is possible to derive
information on the simultaneous model performance of other
variables. @fig-condDat shows the conditional quantile plot
...
```{r condDat,fig.width=15,fig.height=7,fig.keep='last',fig.cap=''}
#| label: fig-condDat
