# A closer look at trends {#trends-gam}
...
for considering trends (`smoothTrend` and `TheilSen`; see Sections \@ref(smoothTrend) and \@ref(TheilSen)), the
...
library(mgcv)
load("~/My Drive/openair/Data/MaceHead.RData")
```
...
```{r MHplot,fig.width=10,fig.height=5,fig.cap='Monthly mean O~3~ concentrations at Mace Head, Ireland (1998--2010).'}
timePlot(monthly, pollutant = "o3")
```

Figure \@ref(fig:MHplot) shows that there is a clear seasonal variation in
...
residuals, shown in Figure \@ref(fig:M0ACF). This Figure clearly shows the
residuals have a strong seasonal pattern. @Chatfield2004
provides lots of useful information on time series modelling.

```{r M0ACF,fig.width=5,fig.height=4,out.width='50%',fig.cap='ACF for the residuals of model M0.'}
acf(residuals(M0))
```
...
```{r M1trend,fig.width=5,fig.height=4,out.width='50%',fig.cap='The trend component of model M1.'}
plot.gam(M1, select = 1, shade = TRUE)
```

```{r M1seas,fig.width=5,fig.height=4,out.width='50%',fig.cap='The seasonal component of model M1.'}
plot.gam(M1, select = 2, shade = TRUE)
```

The seasonal component shown in Figure \@ref(fig:M1seas) clearly shows the
...
\@ref(fig:M1ACF) and shows there is still some short-term
...
Figure \@ref(fig:MHplot). These few points (which skew the residuals) may
...
```{r M1ACF,fig.width=5,fig.height=4,out.width='50%',fig.cap='ACF for the residuals of model M1.'}
acf(residuals(M1))
```
...
The ACF plot is shown in Figure \@ref(fig:M2ACF) and shows that the
...
```{r M2ACF,fig.width=5,fig.height=4,out.width='50%',fig.cap='ACF for the residuals of model M2.'}
...
shown in Figure \@ref(fig:perpsM3). This plot shows for example that
...
```{r perpsM3,fig.width=7,fig.height=7,fig.keep='all',out.width='55%',fig.cap='lot showing the two-way interaction between the trend and seasonal components.'}
