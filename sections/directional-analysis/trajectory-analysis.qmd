...
#| echo: false
## load from file
load("../../assets/data/londonTraj.RData")
```
...
```{r}
#| label: fig-traj4
#| fig-cap: 96-hour Hysplit back trajectories centred on London for 7 days in April 2010, coloured by the concentration of PM~10~ in μg m^-3^.
#| fig-height: 6
#| fig-width: 6
selectByDate(traj,
  start = "15/4/2010",
...
σ~x~(t') = σ~y~(t') = at', a is a dispersion speed, equals to 1.5 km h^-1^. Note that the default σ is assumed to be 1.5 km h^-1^ rather than 5.4 km h^-1^ that is commonly used in other studies because testing shows that a lower value reveals more information about source origins without introducing noise. In practice, the value of σ will vary depending on meteorological conditions and be influenced by both turbulence and wind shear. Users can set their own value of σ in the function options.
...
A *potential mass transfer field* is calculated for a given trajectory l, arriving at time t, is integrated over back trajectory time 𝜏:
...
c~l~: Receptor concentration of back trajectory l
...
#| eval: false
trajLevel(
  dplyr::slice_sample(traj, prop = 0.1),
  pollutant = "pm2.5",
  statistic =  "sqtba",
  map.fill = FALSE,
  cols = "default",
...
#| fig-height: 6
#| fig-width: 10
knitr::include_graphics("../../assets/media/sqtba.png")
```
...
#| fig-height: 6
#| fig-width: 10
knitr::include_graphics("../../assets/media/sqtba2.png")
```
...
#| fig-height: 6
#| fig-width: 10
knitr::include_graphics("../../assets/media/sqtba3.png")
```
...
load("../../assets/data/clusterData.RData")
```
...
kc1 <- importUKAQ(site = "kc1", year = 2011)
```
