...
![Shallow volume source.](../../assets/media/volume-ws.png){#fig-volume-ws}

![Stack source.](../../assets/media/plume-ws.png){#fig-stack-ws}
...
![Behaviour of a dispersing plume from a 60 m stack under different conditions of atmospheric stability. The line corresponds to the mean plume centreline height and the shaded area indicates the mean plume spread (σ~z~).](../../assets/media/stability_plume.png){#fig-atmos-stability}

Just as in @sec-effect-of-source, we can illustrate how concentrations are represented on polar plots when the radial axis is atmospheric stability (H/L~MO~). @fig-volume-stability shows how predicted concentrations vary with atmospheric stability with values close to zero corresponding to neutral conditions and positive values representing stable conditions. For the shallow volume source it is clear that the highest concentrations are observed under stable atmospheric conditions, which tend to also be conditions where the wind speed is low, as shown previously in @fig-volume-ws.
...
![Shallow volume source.](../../assets/media/volume-stability.png){#fig-volume-stability}

![Stack source.](../../assets/media/stack_stability.png){#fig-stack-stability}
...
@fig-polarPlot2 and @fig-polarPlot3 show polar plots using different defaults and for other pollutants. In the first (@fig-polarPlot2), a different colour scheme is used and some adjustments are made to the key. In @fig-polarPlot3, SO~2~ concentrations are shown. What is interesting about this plot compared with either @fig-polarPlot2 or @fig-polarPlot is that the concentration pattern is very different i.e. high concentrations with high wind speeds from the east. The most likely source of this SO~2~ are industrial sources to the east of London. The plot does still however show evidence of a source to the south-west, similar to the plot for NO~x~, which implies that road traffic sources of SO~2~ can also be detected.
...
## Nonparametric Wind Regression, NWR {#sec-polar-NWR}
...
As an example of usage we will consider the relationship between PM~2.5~ and PM~10~ at the rural Harwell site in Oxfordshire. Additionally, we will use meteorological data from a nearby site rather than rely on modelled values that are provided in `importUKAQ()`.
...
Note that if you are using the `importUKAQ()` function it is useful to add the option `meta = TRUE`, which will return the latitude and longitude of the site together with the air quality data.
...
For more information about using [openairmaps]{.pkg} to create maps with polar plots as markers, please refer to the [Directional Analysis Maps Page](../maps/maps-polar.qmd).
