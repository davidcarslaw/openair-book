...
`selectByDate` function and plot NO~x~ vs. NO~2~ 
...
records. In this case @fig-scatterPlot2 provides a clearer
indication of the relationship between NO~x~ and NO~2~ than @fig-scatterPlot1 because it reveals where most of the points
lie, which is not apparent from @fig-scatterPlot1. Note that
...
lie. This is possible in `scatterPlot` with the `method =
  "density"` option. Such a plot is shown in @fig-scatterPlotDens.
...
variable. @fig-scatterPlot4 shows again the relationship
...
@fig-scatterPlot4 was very easily produced
...
@fig-scatterPlot5 shows that `scatterPlot` can also
...
Similar to @sec-timePlot, `scatterPlot` can
also plot wind vector arrows if wind speed and wind direction are
available in the data frame. @fig-scatterPlotwswd shows an
...
function. @fig-scatterPlotwswd shows that when the wind
