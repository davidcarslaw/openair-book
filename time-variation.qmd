...
-  The function works well in conjunction with other functions
  such as `polarPlot` (see @sec-polarPlot), where the latter may identify conditions
...
(@sec-polarPlot) is very useful in this respect in highlighting
...
The results are shown in @fig-timeVar1. The plot shown at
...
@fig-timeVar2 shows the function applied to
...
changed over these two periods. The results are shown in @fig-timeVar3. There is some indication in this plot
...
There are a couple of things to note in @fig-timeVar2. There seems to be evidence that for
...
species of required. An example is shown in @fig-timeVarMedian for PM~10~ concentrations.

```{r}
#| label: fig-timeVarMedian
#| fig-cap: Example plot using the `timeVariation` function to show the variation in the median, 25/75th and 5/95th quantile values for PM~10~. The shading shows the extent to the 25/75th and 5/95th  quantiles.
#| fig-width: 12
#| fig-height: 7
