...
Alternatively it may be advantageous to use surface measurements. [worldmet]{.pkg} provides an easy way in which to access surface
meteorological data from >30,000 sites across the world [@R-worldmet]. The package
accesses the NOAA webservers to download hourly data. See https://github.com/davidcarslaw/worldmet and https://www.ncdc.noaa.gov/isd for further information.

Access to
surface meteorological data is very useful in general but is
especially useful when using [openair]{.pkg} and functions such as `polarPlot`. To install the package, type:
...
As an example, we will search for the 10 nearest sites to Dublin (latitude =
