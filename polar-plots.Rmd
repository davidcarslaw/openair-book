...
given by Equation \@ref(eq:mod}. In this model the square root-transformed
...
with infinity and the [openair]{.pkg} functions will remove these values
...
intervals. In the approach adopted in [openair]{.pkg} (which is not identical
to @henry2009), Gaussian smoothers are used for both wind
direction and wind speed. Unlike the default GAM approach in [openair]{.pkg},
the NWR technique works directly with the raw (often hourly) data. It
tends to provide similar results to [openair]{.pkg} but may have advantages
...
a particular layout see Section \@ref(sec:multiple-plots-page).
...
features together. One such approach is to use *cluster analysis*. [openair]{.pkg} uses k-means clustering as a way in which
...
conditions together. Section \@ref(sec:trajCluster) provides the details of clustering back trajectories in [openair]{.pkg}. A fuller description of the
clustering approach is described in @CarslawClust2013.

The use of the `polarCluster` is very similar to the use of all
[openair]{.pkg} functions. While there are many techniques available to try
...
Now other [openair]{.pkg} analysis functions can be used to analyse the
...
using other [openair]{.pkg} functions or indeed other analyses altogether.
...
A developmental R package called [openairmaps{.pkg} has been developed to
plot bivariate polar plots on an interactive leaflet map. This package
is not on CRAN but can be installed from GitHub:
...
The function `polarMap` requires information on the site (anme or code), latitude and
longitude in addition to the usual information required by the
`polarPlot` function. The package comes with some example data
(`polar_data`) that can be used as a template for using different
data.

The function can plot *any* type of polar plot e.g. with different options for `statistic`.
...
Or with a different base map (there are many!). See the help file for `polarMap` on how to find out about and use different base maps. Note that if you are using the `importAURN` family of functions it is useful to add the option `meta = TRUE`, which will return the latitude and logitude of the site together with the air quality data.

```{r interMap2,cache=FALSE}
polarMap(polar_data, 
         latitude = "latitude", 
         longitude = "longitude",
         type = "site", 
