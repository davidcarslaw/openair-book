...
@fig-windRoseyear highlights some interesting differences
...
quickly consider the dependencies between variables. @sec-cutData) describes the basis of this option in [openair]{.pkg}
...
set by the user --- see `?cutData` for more details. @fig-windRsPM10 shows the results of setting `type = "pm10"`. For
...
function in recent versions of [openair]{.pkg}, e.g. as in @fig-pollutionRoseBasic:
...
While @fig-pollutionRoseBasic indicates that higher NO~x~
concentrations are also associated with the SW, conditioning allows you
to be much informative. For example, conditioning by SO~2~ (@fig-pollRoseNOxO2) demonstrates that higher NO~x~ concentrations
...
most apparent at highest SO~2~ concentrations that is obscured in @fig-pollutionRoseBasic by a relatively high NO~x~ background @fig-pollRoseNOxO2.
...
different concentration levels. A simple plot is shown in @fig-pollRoseAttr, which clearly shows the dominance of
...
clearly. An example is shown in @fig-pollRoseNorm where for
...
results are shown in @fig-pollRoseComp. The Figure clearly
...
negatively biased shown in blue. Also shown in @fig-pollRoseComp is the mean wind speed and direction bias as numerical values.

Note that the `type` option can be used in @fig-pollRoseComp e.g. `type = "month"` to split the analysis in
...
An example of using user-supplied breaks is shown in @fig-pollRoseComp2. In this case six intervals are chosen including one that spans -0.5 to +0.5 that is useful to show wind speeds that do not change.
