...
bootstrap simulations. Note also that the all the regression parameters
...
the median of all these slopes. The advantage of the using the Theil-Sen
...
is there any evidence that concentrations of \nox have decreased since
...
`timeVariation` function are both useful to determine whether there is a
...
methods. The overall trend is shown at the top-left as 0.38 (ppb) per
year and the 95% confidence intervals in the slope from 0.21--0.51
...
These printed results show that in this case the trend in O~3~ was +0.38
units (i.e. ppb) per year as an average over the entire period. It also
shows the 95% confidence intervals in the trend ranged between 0.21 to
0.51 ppb/year. Finally, the significance level in this case is very
...
and start date, respectfully. $N_{years}$ is the number of years (or
