...
\begin{equation}
   0.5 \leq \frac{M_i}{O_i} \leq 2.0 (#eq:FAC2)
\end{equation}
...
\begin{equation}
 MB = \frac{1}{n}\sum_{i=1}^{N} M_i - O_i  (#eq:MB)
\end{equation}
...
\begin{equation}
 MGE = \frac{1}{n}\sum_{i=1}^{N} |M_i - O_i|   (#eq:MGE)
\end{equation}
...
\begin{equation}
 NMB = \frac{\sum\limits_{i=1}^{n} M_i - O_i}{\sum\limits_{i=1}^{n} O_i} (#eq:NMB)
\end{equation}
...
\begin{equation}
 NMGE = \frac{\sum\limits_{i=1}^{n} |M_i - O_i|}{\sum\limits_{i=1}^{n} O_i}  (#eq:NMGE)
\end{equation}
...
\begin{equation}
   RMSE = \left({\frac{\sum\limits_{i=1}^{n} (M_i - O_i)^2}{n}}\right)^{1/2}   (#eq:RMSE)
\end{equation}
...
\begin{equation}
 r = \frac{1}{(n-1)}\sum\limits_{i=1}^{n}\left(\frac{M_i - \overline{M}}{\sigma_M}\right)\left(\frac{O_i - \overline{O}}{\sigma_O}\right)   (#eq:r)
\end{equation}
...
\begin{equation}
 COE = 1.0 - \frac{\sum\limits_{i=1}^{n} |M_i
             -O_i|}{\sum\limits_{i=1}^{n} |O_i - \overline{O}|}   (#eq:COE)
\end{equation}
...
```{r showModData}
load("~/My Drive/openair/Data/modelData.RData")
head(modTest)
```
